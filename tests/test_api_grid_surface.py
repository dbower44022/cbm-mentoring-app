...
- REQ-023: the selection wire shape round-trips, select-all means the whole
...
- REQ-027: the selection-else-filtered rule, format gating, and the job
  payload carrying the view rendering + the artifact_jobs job types.
...
the surface is generic; nothing here is postal-specific.
"""
...
from mentorapp.storage import PostalCode, SchemaRegistry, utcnow
...
def test_selection_wire_shape_round_trips() -> None:
    picked = uuid.uuid4()
    explicit = parse_selection({"selectionKind": "explicit", "recordIds": [str(picked)]})
    assert explicit == ExplicitSelection((picked,))
...
@pytest.mark.parametrize(
    ...
    [
        ...
        {},
        {"selectionKind": "explicit", "recordIds": ["not-a-uuid"]},
    ],
)
...
def test_selection_filters_compose_with_the_grid_filters(session: Session) -> None:
    ...
    explicit = selection_record_filter(ExplicitSelection((made[0].postal_code_id,)), PostalCode)
    assert explicit is not None
    rows, _ = keyset_page(
        session, PostalCode, sort_field="postalCodeValue", page_size=10, filters=[explicit]
    )
    assert [row.postal_code_id for row in rows] == [made[0].postal_code_id]
    # Select-all with nothing deselected adds NO predicate — it IS the filtered set.
    assert selection_record_filter(FilteredSetSelection(()), PostalCode) is None
    minus_one = selection_record_filter(
        FilteredSetSelection((made[1].postal_code_id,)), PostalCode
    )
    assert minus_one is not None
    rows, _ = keyset_page(
        session, PostalCode, sort_field="postalCodeValue", page_size=10, filters=[minus_one]
    )
    assert made[1].postal_code_id not in {row.postal_code_id for row in rows}
    assert len(rows) == 2
...
def test_hidden_selection_count_and_confirmation_wording(session: Session) -> None:
    ...
    selected = [row.postal_code_id for row in made]
    hidden = hidden_selection_count(
        session, PostalCode, selected, filters=[PostalCode.state_code == "IL"]
    )
    assert hidden == 1  # the Oregon row stays selected but filtered out
    assert hidden_selection_count(session, PostalCode, [], filters=[]) == 0
...
def test_selection_else_filtered_rule() -> None:
    picked = uuid.uuid4()
    assert resolve_export_scope(ExplicitSelection((picked,))) == ExportScope(
        "selection", record_ids=(picked,)
    )
...
def test_print_payload_shares_the_scope_contract_without_format() -> None:
    ...
    job_type, payload = print_job_payload(
        _document_request(scope=resolve_export_scope(ExplicitSelection((picked,))))
    )
    assert job_type == PRINT_JOB_TYPE
    assert payload["scope"] == {"scopeKind": "selection", "recordIds": [str(picked)]}
...
def test_system_view_and_own_view_open_as_named() -> None:
    ...
    for link in (system_link, own_link):
        outcome = resolve_grid_link(
            link, requester_id=me, has_data_source_access=True, last_used_view_id=None
        )
        assert outcome == OpenLinkedView(link.view_id)


def test_another_users_private_view_falls_back_with_notice() -> None:
    last_used = uuid.uuid4()
    outcome = resolve_grid_link(
        _link(uuid.uuid4()),
        requester_id=uuid.uuid4(),
        has_data_source_access=True,
        last_used_view_id=last_used,
    )
    assert isinstance(outcome, FallbackToLastUsed)
    assert outcome.view_id == last_used and "last-used" in outcome.notice
    # No last-used view yet: the grid still opens (on its default), never blank.
